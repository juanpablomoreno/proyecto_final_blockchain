...
// Función para poner en venta un NFT.
    function putOnSale(uint256 tokenID, uint256 price) external { // Parámetros: ID y precio del token.
        require(canTrade == false, "Trade is not allowed"); // Verificación de operaciones de comercio (canTrade). Incluir un mensaje de falla.

        require(_exists(tokenID), "Token ID doesn't exists"); // Verificción de existencia del tokenId mediante "_exists". Incluir un mensaje de falla.
